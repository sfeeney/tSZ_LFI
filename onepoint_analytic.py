...
# complete C_ell's, comparison to MassiveNus and Hu
if False :#{{{
    name = './testsApr30/'
    cosmo = cosmology(
        {
...
#        'As': 2.3e-9,
    ...
    num = numerics(
        ...
        'Npoints_z': 200,
        'Npoints_small_M': 201,
        'sigma_chi_file': './constants/MassiveNus_Mvir_sigma_chi_file.npz',
        'signal_type': 'kappa',
#        'pixel_sidelength': (np.pi/180.)*0.95/60.
        }
        )
    ...
    Hu_ell1, Hu_C1 = np.loadtxt('./Hu_WL_powerspectra/one_halo.csv', unpack = True, delimiter = ',')
    Hu_ell2, Hu_C2 = np.loadtxt('./Hu_WL_powerspectra/two_halo.csv', unpack = True, delimiter = ',')
    sortindices = np.argsort(M_ell)
    M_ell = M_ell[sortindices]
    MC_ell = MC_ell[sortindices]
    sortindices = np.argsort(H_ell)
    H_ell = H_ell[sortindices]
    HC_ell = HC_ell[sortindices]
#    plt.loglog(num.larr, p.Ckappa_ell, label = 'total', linewidth = 4)
#    plt.loglog(num.larr, p.onehalo_term, label = 'one halo')
#    plt.loglog(num.larr, p.twohalo_term, label = 'two halo')
    plt.loglog(num.larr, p.Ckappa_ell*num.larr*(num.larr+1.), label = 'total', linewidth = 4)
    plt.loglog(num.larr, p.onehalo_term*num.larr*(num.larr+1.), label = 'one halo')
    plt.loglog(num.larr, p.twohalo_term*num.larr*(num.larr+1.), label = 'two halo')
#    plt.loglog(num.larr, p.halofit_Ckappa_ell[::-1], label = 'halofit', linestyle = 'dashed', linewidth = 4)
#    plt.loglog(H_ell, 2.*np.pi*HC_ell/H_ell/(H_ell+1.), marker = 'o', label = 'Halofit')
#    plt.loglog(M_ell, 2.*np.pi*MC_ell/M_ell/(M_ell+1.), marker = 'o', label = 'MassiveNus')
    plt.loglog(H_ell, 2.*np.pi*HC_ell, marker = 'o', label = 'Halofit')
    plt.loglog(M_ell, 2.*np.pi*MC_ell, marker = 'o', label = 'MassiveNus')
#    plt.loglog(Hu_ell1, 2.*np.pi*Hu_C1, marker = 'o', label = 'Hu 1 halo')
#    plt.loglog(Hu_ell2, 2.*np.pi*Hu_C2, marker = 'o', label = 'Hu 2 halo')
    plt.legend(loc = 'upper left')
    plt.xlim(min(M_ell), max(M_ell))
    plt.show()
    #########
#    interp1 = interp1d(num.larr, p.onehalo_term, bounds_error = False, fill_value = 0.)
#    interp2 = interp1d(num.larr, p.twohalo_term, bounds_error = False, fill_value = 0.)
#    atHuell1 = interp1(Hu_ell1)
#    atHuell2 = interp2(Hu_ell2)
#    plt.semilogx(Hu_ell1, (Hu_ell1*(Hu_ell1+1.)*atHuell1/Hu_C1/2./np.pi), label = '1 halo')
#    plt.semilogx(Hu_ell2, (Hu_ell2*(Hu_ell2+1.)*atHuell2/Hu_C2/2./np.pi), label = '2 halo')
#    plt.legend()
#    plt.show()
    #########
    print (p.small_mass_kappa_variance)
#}}}
...
if False :#{{{
    ...
    #cosmo.create_HMF_and_bias(name, num)
    #pr = profiles(cosmo, num)
    #pr.create_profiles(name)
    #pr.create_convolved_profiles(name)
    #pr.create_tildes(name)
    #p = PDF(cosmo, num, pr)
    #m = signal_map(cosmo, num, pr, {'exact_cluster_number': True})
    #bin_edges = np.linspace(0., 0.35, num = 100)
    #for ii in xrange(100,130) :
    #    start = time()
    #    print ii
    #    m.create_final_map(name)
    #    m.create_histogram(bin_edges, name, ii)    
    #    end = time()
    #    print str((end-start)/60.) + ' minutes for this run.'
    #    p.create_P_Poisson(name, ii)
    #p.create_alpha0(name)
    #p.create_P_tilde(name)
    #p.create_P(name, {'gaussian_piece': False})
    h = Poisson_histograms(name)
    h.read_map_histograms()
    h.read_exact_map_histograms()
    h.read_P_histograms()
    #h.get_averages()
    h.get_2nd_moment()
    ...
    ax.semilogy(h.bin_centres, h.map_var, label = 'complete variance')
    ax.semilogy(h.bin_centres, h.exact_map_var, label = 'placement variance')
    ax.semilogy(h.bin_centres, h.P_var, label = 'Poisson variance')
    ax.legend(loc = 'upper left')
    ax.set_xlabel('convergence $\kappa$')
    ax.set_ylabel('variance')
    plt.savefig('variance_contributions.pdf', bbox_inches = 'tight')
    plt.show()
    #########################
    #h.get_nth_moment(3)
#}}}
...
if False :
    ...
    num = numerics(
        {
        'debugging': True,
        'sigma_chi_file': './constants/Batt_cosmo_sigma_chi_file.npz',
        'signal_type': 'tSZ',
        'Npoints_theta': 1000,
        'pixel_sidelength': (np.pi/180.)*0.24/60.,
        'z_max': 1.
        }
        )
    #cosmo.create_HMF_and_bias(name, num)
    ...
    #pr.create_convolved_profiles(name)
    #pr.create_tildes(name)
    p = PDF(cosmo, num, pr, {'nu_obs': 148e9})
    #p.create_alpha0(name)
    #p.create_P_tilde(name)
    #p.create_P(name)
    fig,ax = plt.subplots()
    line_sim, line_FT = p.compare_to_sims(name, './', ax, 1.)
    plt.show()
#}}}

# Plot for Colin (all source redshifts)
if False :#{{{
    ...
    fig,ax = plt.subplots(nrows = 2, ncols = 3, figsize = (20,15), gridspec_kw={'wspace':0.})
    fudge_factors = [0.65, 0.73, 0.8, 0.825, 0.85]
    xmin = [-0.02, -0.05, -0.08, -0.1, -0.13]
    xminp = [0.0, 0.0, 0.0, 0., -0.1]
    xmaxp = [0.12, 0.3, 0.4, 0.6, 0.7]
    nums = [5,3,5,7,5]
    xmax = [0.15, 0.35, 0.5, 0.7, 0.8]
    #ymin = [1e-6, 1e-7, 1e-7, 1e-8, 5e-9]
    ymin = [1e-7, 1e-7, 1e-7, 1e-9, 5e-9]
    #ymax = [3e-1, 2e-1, 2e-1, 2e-1, 2e-1]
    ymax = [3e-1, 3e-1, 3e-1, 2e-1, 2e-1]
    zs = ['0.5', '1.0', '1.5', '2.0', '2.5']

    for ii,source_redshift in enumerate(zs) :
        name = './testsApr26/zs' + source_redshift + '/'
        #cosm_params = {}
        #num_params = {}
        cosmo = cosmology(
    #        {
    #        'external_HMF_file': './MassiveNus_HMF_from_Colin.npz'
    #        }
            )
        num = numerics(
            {
            ...
            #'pixel_radius': (np.pi/180.)*0.2/60. # in radians -- Pixel sidelength is 0.41 arcmin
            'pixel_sidelength': (np.pi/180.)*0.41/60.,
            'Npoints_theta': 1500,
            'signal_type': 'kappa'
            }
        ...
        #p.create_P_tilde(name)
        #p.create_P(name)
        line_sim, line_FT, line_FT_fudge = p.compare_to_sims(name, './MassiveNus_sims/', ax.flatten()[ii], fudge_factors[ii])
        ax.flatten()[ii].text(0.95,0.95, 'source redshift = ' + source_redshift, ha = 'right', va = 'top', transform = ax.flatten()[ii].transAxes)
        ax.flatten()[ii].text(0.95,0.87, 'scaling = ' + str(fudge_factors[ii]), ha = 'right', va = 'top', transform = ax.flatten()[ii].transAxes)
        ax.flatten()[ii].set_xlabel('convergence $\kappa$')
        if ii%3 == 0 :
            ax.flatten()[ii].set_ylabel('PDF $p_{\kappa}$')
        else :
            ax.flatten()[ii].set_yticklabels([])
        ax.flatten()[ii].set_xlim(xmin[ii], xmax[ii])
        ax.flatten()[ii].set_ylim(ymin[ii], ymax[ii])
        ax.flatten()[ii].set_xticks(np.round(np.linspace(xminp[ii],xmaxp[ii],num = nums[ii]),2))
    ax.flatten()[-1].axis('off')
    fig.legend((line_sim[0], line_FT[0], line_FT_fudge[0]), ('simulation', 'analytic result', 'scaled result'), loc = 'upper left', bbox_to_anchor = (0.,1), bbox_transform = ax.flatten()[-1].transAxes)
    #fig.tight_layout()
    plt.savefig('all_source_redshifts_both.pdf', bbox_inches = 'tight')
    plt.show()
#}}}
...
if False :#{{{
    ...
    num = numerics(
        ...
        'z_source': 1.,
        'Npoints_theta': 1000,
        'physical_smoothing_scale': 1e-2, # Mpc/h
        'pixel_sidelength': (np.pi/180.)*(0.41/60.),
        'smoothing_Npoints_scale': 4.
        }
        )
    pr = profiles(cosmo,num)
    pr.create_profiles(name)
